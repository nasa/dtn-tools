...
class CanonicalBlock(Block):
    ...
    def __init__(
        self, blk_type=None, blk_num=None, control_flags=None, crc_type=None, crc=None
    ):
        ...
        if not isinstance(blk_num, int):
            warnmsg = (
                f"{cclass_name} blk_num is type {type(blk_num).__name__} instead of int"
            )
            warnings.warn(warnmsg, TypeWarning)
        self.blk_num = blk_num

        if not isinstance(control_flags, BlockPCFlags) and not isinstance(
            control_flags, int
        ):
        ...
        if (
            not isinstance(crc, CRCFlag)
            and not isinstance(crc, bytes)
            and crc is not None
        ):
    ...
    def encode(self, type_spec_data=None):
        ...
        if (
            self.crc is CRCFlag.CALCULATE
            and self.crc_type is not None
            and self.crc_type >= 1
            and self.crc_type <= 2
        ):
            tmp = [
    ...
    def to_json(self, type_spec_key, type_spec_data=None):
        ...
        if type_spec_data is not None:
            if not isinstance(type_spec_data, bytes):
                enc_type_spec_data = cbor2.dumps(
                    type_spec_data, default=default_encoder
                )
    ...
    def to_json(self):
        ...
        *Usage:*
        ...
        return super().to_json(
            type_spec_key="prev_eid", type_spec_data=self.get_type_spec()
        )

    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Previouse Node Block.
...
calculate it
    ...
    def to_json(self):
        ...
        *Usage:*
        ...
        """
        return super().to_json(
            type_spec_key="bundle_age", type_spec_data=self.get_type_spec()
        )

    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Bundle Age Block.
...
node to another
    ...
    def to_json(self):
        ...
        *Usage:*
        ...
        """
        return super().to_json(
            type_spec_key="hop_data", type_spec_data=self.get_type_spec()
        )

    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Hop Count Block.
...
reg_orig_eid EID of the originator of the custody request
    ...
    def to_json(self):
        ...
        *Usage:*
        ...
        """
        return super().to_json(
            type_spec_key="cteb_data", type_spec_data=self.get_type_spec()
        )

    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Custody Transfer Extension Block.
...
class CompressedReportingBlock(CanonicalBlock):
    ...
    def to_json(self):
        ...
        *Usage:*
        ...
        """
        return super().to_json(
            type_spec_key="creb_data", type_spec_data=self.get_type_spec()
        )

    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Compressed Reporting Extension Block.
...
class PayloadBlock(CanonicalBlock):
    ...
    def __init__(
        ...
        r"""Initialize the payload block with the requested fields.
    ...
    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Payload Block.
...
creation time at which the bundle's payload will no longer be useful
        ...
        *Usage:*
        ...
        if not isinstance(version, int):
            warnmsg = (
                f"PrimaryBlock version is type {type(version).__name__} instead of int"
            )
            warnings.warn(warnmsg, TypeWarning)
        self.version = version

        if not isinstance(control_flags, BundlePCFlags) and not isinstance(
            control_flags, int
        ):
        ...
        if not isinstance(src_eid, EID):
            warnmsg = (
                f"PrimaryBlock src_eid is type {type(src_eid).__name__} instead of EID"
            )
            warnings.warn(warnmsg, TypeWarning)
        ...
        if not isinstance(rpt_eid, EID):
            warnmsg = (
                f"PrimaryBlock rpt_eid is type {type(rpt_eid).__name__} instead of EID"
            )
            warnings.warn(warnmsg, TypeWarning)
    ...
    @classmethod
    def decode(cls, cand_block):
        r"""Attempt to decode the candidate block as a BPv7 Primary Block.
        ...
        *Usage:*
        ...
        if blen > 9:
            warnmsg = (
                "Primary block length greater than 9 elements. Truncating to 9 elements"
            )
            warnings.warn(warnmsg)
...
class PrimaryBlockSettings:
    ...
    def __init__(
        ...
        r"""Initialize the primary block with the requested fields.

        :param int version: (optional) version of the Bundle Protocol that constructed this block
        :param BundlePCFlags control_flags: (optional) bundle processing control flags
        :param CRCType crc_type: (optional) CRC type
        :param EID dest_eid: (optional) bundle endpoint that is the bundle's destination
        :param EID src_eid: (optional) bundle node at which the bundle was initially transmitted, or null endpoint if anomymous
        :param EID rpt_eid: (optional) bundle endpoint to which status reports pertaining to the forwarding and delivery of this bundle are to be transmitted
        ...
        :param int lifetime: (optional) number of milliseconds past the creation time at which the bundle's payload will no longer be useful
        :param bytes crc: (optional) crc value or CRCFlag.CALCULATE to calculate it
        ...
        *Usage:*
        ...
        if creation_timestamp and isinstance(creation_timestamp, dict):
            if (
                isinstance(creation_timestamp["time"], str)
                and creation_timestamp["time"].lower() == "current"
            ):
                self.cts_time_current = True

            if not self.cts_time_current and isinstance(
                creation_timestamp["time"], dict
            ):
                if "start" in creation_timestamp["time"] and isinstance(
                    creation_timestamp["time"]["start"], int
                ):
                    self.cts_time_start = creation_timestamp["time"]["start"]
                if "increment" in creation_timestamp["time"] and isinstance(
                    creation_timestamp["time"]["increment"], int
                ):
                    self.cts_time_increment = creation_timestamp["time"]["increment"]

            if "sequence" in creation_timestamp and isinstance(
                creation_timestamp["sequence"], dict
            ):
                if "fixed" in creation_timestamp["sequence"]:
                    self.cts_seq_fixed = True
                    if isinstance(creation_timestamp["sequence"]["fixed"], int):
                        self.cts_seq_start = creation_timestamp["sequence"]["fixed"]
                elif "start" in creation_timestamp["sequence"] and isinstance(
                    creation_timestamp["sequence"]["start"], int
                ):
                    self.cts_seq_start = creation_timestamp["sequence"]["start"]

        else:
            raise ValueError(
                "creation_timestamp not provided or not a dict with settings"
            )

        if not self.cts_time_current:
            if not (self.cts_time_start and self.cts_time_increment):
                raise ValueError(
                    'creation_timestamp "time" is not "current" and "start" and/or "increment" not provided or are not ints'
                )

        if self.cts_seq_fixed:
            if self.cts_seq_start is None:
                raise ValueError(
                    'creation_timestamp "sequence" type is fixed, but "fixed" value is not an int'
                )
        else:
            if self.cts_seq_start is None:
                raise ValueError(
                    'creation_timestamp "sequence" "start" value is not an int'
                )

    def generate(self, bundle_num):
        r"""Generate a Primary Block with the settings in this PayloadBundleSettings and the given bundle number.
        ...
        *Usage:*
        ...
        generated_seq = self.cts_seq_start + (0 if self.cts_seq_fixed else bundle_num)
        ...
        return PrimaryBlock(
            ...
            creation_timestamp=CreationTimestamp(
                {"time": generated_time, "sequence": generated_seq}
            ),
...
class UnknownBlock(Block):
    ...
    def __init__(
        ...
        r"""Initialize the unknown block with the requested fields.
        ...
        *Usage:*
        ...
        if isinstance(self.elements, list):
            fields = [self.__class__.__name__] + [
                i for i in self.elements if i is not None
            ]
        else:
    ...
    @classmethod
    def decode(cls, cand_block):
        r"""Decode the candidate block as an Unknown Block.
