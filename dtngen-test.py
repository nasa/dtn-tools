from dtngen import Bundle
from dtngen import BundlePCFlags, CRCType, EID, BlockType, \
    CreationTimestamp, BlockPCFlags, CRCFlag
from dtngen import PrimaryBlock, PayloadBlock, PrevNodeBlock, \
...
# Step 5: Define new primary and payload blocks
...
assert bfromfile.canon_blocks[1].control_flags == \
...
assert bfromfile.canon_blocks[2].control_flags == \
    BlockPCFlags.FRAG_REPLICATE
...
assert bfromfile.canon_blocks[3].payload == b'\x00\x00\x00\x00\x00\x00\x00\x0chello world\n'
...
assert bfrombytes.canon_blocks[1].control_flags == \
    BlockPCFlags.FRAG_REPLICATE | BlockPCFlags.DEL_UNPROC
...
assert bfrombytes.canon_blocks[2].control_flags == \
    BlockPCFlags.FRAG_REPLICATE
...
assert bfrombytes.canon_blocks[3].payload == b'\x00\x00\x00\x00\x00\x00\x00\x0chello world\n'
...
assert read_bundle.canon_blocks[1].control_flags == \
    BlockPCFlags.FRAG_REPLICATE | BlockPCFlags.DEL_UNPROC
...
assert read_bundle.canon_blocks[2].control_flags == \
    BlockPCFlags.FRAG_REPLICATE
...
assert read_bundle.canon_blocks[3].payload == b'\x00\x00\x00\x00\x00\x00\x00\x0chello world\n'
